%experiment Verkippung in Bildebene
...
for i = 1:1:10
    ...
    filea = strcat('..\..\images\Verkippung\',num2str(j),'default.bmp');
    A=double(imread(filea));
    fileb = strcat('..\..\images\Verkippung\',num2str(j),'.bmp');
...
filename = 'messdateiVerkippung.xlsx';
col_header = {'Verkippung in mm','Verschiebung X [um]','Y [um]','Messwert X [um]','Y [um]','Differenz X [um]','Y [um]','Standardabweichung X [um]','Y [um]','Relativer Fehler X','Y', 'Konfidenzintervall X [um]','','Konfidenzintervall Y [um]',''};
